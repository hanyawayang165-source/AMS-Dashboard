...
(function initGasRunAdapter_() {
  ...
  // WAJIB DIISI: URL hasil Deploy > Manage deployments > Web app di Apps Script Editor.
  // Contoh: 'https://script.google.com/macros/s/AKfycb.../exec'
  var GAS_WEBAPP_URL_ = 'ISI_URL_DEPLOY_APPS_SCRIPT_ANDA_DI_SINI';
  ...
  getMonthlyTrendYear: ['tahun', 'token'],
  ...
  exportSheetPdf: ['sheetName', 'token', 'department'],
  getUsersList: ['token'],
  getRolesList: ['token'],
  ...
  setRolePermissions: ['roleName', 'menuKeys', 'editMenuKeys', 'token'],
  ...
  function callGasAction_(actionName, args, onSuccess, onFailure) {
    ...
    if (!paramNames) {
      const msg = 'Adapter Netlify: aksi "' + actionName + '" belum terdaftar di GAS_PARAM_MAP_ (JavaScript.html).';
      console.error(msg);
...
const TAB_TITLES_ = {
  ...
  function selectTab_(tabName) {
    ...
    if (titleEl) titleEl.textContent = TAB_TITLES_[tabName] || 'Dashboard';
        const breadcrumbEl = document.getElementById('breadcrumbTabName');
    if (breadcrumbEl) breadcrumbEl.textContent = TAB_TITLES_[tabName] || 'Dashboard';
...
function onDepartmentFilterChange() {
  ...
  function renderAuthArea() {
    const el = document.getElementById('authArea');
        const bcUser = document.getElementById('breadcrumbUserName');
    if (bcUser) bcUser.textContent = SESSION ? SESSION.nama : '-';
    if (SESSION) {
      el.innerHTML = `
  ...
  function updateInputEditability() {
    ...
    const canEditInputPermission = !!MY_PERMISSIONS_.edit && MY_PERMISSIONS_.edit.indexOf('input') !== -1;
    const canEdit = !!SESSION && canEditInputPermission && !isArchiveView;
    ...
    if (hint) {
      if (isArchiveView) {
      ...
      } else if (SESSION && canEditInputPermission) {
        ...
        hint.textContent = 'Role Anda (' + SESSION.role + ') hanya bisa MELIHAT data Realisasi, tidak bisa mengubahnya.';
      } else {
  ...
  function renderAll() {
    ...
    const realisasiEditable = !!SESSION && MY_PERMISSIONS_.edit && MY_PERMISSIONS_.edit.indexOf('input') !== -1 && currentViewPeriodKey === 'active';
renderGrid('realisasiWrap', STATE.realisasi, STATE.dayCount, STATE.dayNames, STATE.statusCodes, realisasiEditable, 'realisasi');
  ...
  /* ============ TREN KEHADIRAN BULANAN (Dashboard) ============ */

  function loadTrenBulanan_() {
    if (!SESSION) return;
    const tahun = new Date().getFullYear();
    google.script.run
      .withSuccessHandler(data => renderTrenBulanan_(data))
      .withFailureHandler(err => {
        const hint = document.getElementById('trenBulananHint');
        if (hint) hint.textContent = 'Gagal memuat tren bulanan: ' + err.message;
      })
      .getMonthlyTrendYear(tahun, SESSION.token);
  }

  function renderTrenBulanan_(data) {
    if (!data || !data.labels || !data.persenHadir) return;

    const persen = data.persenHadir.filter(v => typeof v === 'number');
    const rata = persen.length ? (persen.reduce((s, v) => s + v, 0) / persen.length) : 0;
    const tertinggi = persen.length ? Math.max(...persen) : 0;
    const terendah = persen.length ? Math.min(...persen) : 0;

    const cardsEl = document.getElementById('trenBulananCards');
    if (cardsEl) {
      cardsEl.innerHTML = `
        <div class="card">
          <div class="card-label">Rata-rata % Hadir</div>
          <div class="card-value">${rata.toFixed(1)}%</div>
        </div>
        <div class="card">
          <div class="card-label">Tertinggi</div>
          <div class="card-value" style="color:#22c55e">${tertinggi.toFixed(1)}%</div>
        </div>
        <div class="card">
          <div class="card-label">Terendah</div>
          <div class="card-value" style="color:#ef4444">${terendah.toFixed(1)}%</div>
        </div>
      `;
    }
    ...
    if (chartTrenBulanan) chartTrenBulanan.destroy();
    chartTrenBulanan = new Chart(canvas, {
      type: 'line',
      data: {
        labels: data.labels,
        datasets: [
          { label: '% Hadir', data: data.persenHadir, borderColor: '#2563eb', backgroundColor: '#2563eb', tension: 0.25, pointRadius: 3, fill: false }
        ]
      },
      options: {
        responsive: true,
    ...
    const hint = document.getElementById('trenBulananHint');
    if (hint) hint.textContent = 'Menampilkan tren % kehadiran per bulan sepanjang tahun ' + new Date().getFullYear() + '.';
  }
  ...
  function renderCards() {
    ...
    renderAbsenPieStats_(totalAlfa, totalIzin, totalSakit, total);
  ...
  function renderAbsenPieStats_(totalAlfa, totalIzin, totalSakit, total) {
    const el = document.getElementById('absenPieStats');
    if (!el) return;
    const rows = [
      { label: 'Alfa', value: totalAlfa, color: '#ef4444' },
      { label: 'Izin', value: totalIzin, color: '#3b82f6' },
      { label: 'Sakit', value: totalSakit, color: '#f59e0b' }
    ];
    el.innerHTML = rows.map(r => {
      const pct = total > 0 ? ((r.value / total) * 100).toFixed(1) : '0.0';
      return `
        <div class="absen-stat-row">
          <span class="absen-stat-label">
            <span class="absen-stat-dot" style="background:${r.color}"></span>
            ${r.label}
          </span>
          <span>
            <span class="absen-stat-value" style="color:${r.color}">${r.value}</span>
            <span class="absen-stat-pct">(${pct}%)</span>
          </span>
        </div>`;
    }).join('') + `
      <div class="absen-stat-row" style="background:#eef2ff;border-color:#c7d2fe;">
        <span class="absen-stat-label">Total Tidak Hadir</span>
        <span class="absen-stat-value">${total}</span>
      </div>`;
  }
...
document.getElementById('newEmployeeDeptSearch').value = '';
document.getElementById('newEmployeeDeptOptions').classList.add('hidden');
if (!DAFTAR_KARYAWAN_DATA.length) loadDaftarKaryawan();
    ...
    /* ---- Searchable Dropdown Department (kunci ke daftar yang sudah ada) ---- */

  let deptSelectionInProgress_ = false;

  function getUniqueDepartmentsList_() {
    return Array.from(new Set(
      (DAFTAR_KARYAWAN_DATA || []).map(k => (k.department || '').trim()).filter(Boolean)
    )).sort();
  }

  function renderDeptSearchOptions_(query) {
  const box = document.getElementById('newEmployeeDeptOptions');
  const all = getUniqueDepartmentsList_();
  const q = (query || '').trim();
  const qLower = q.toLowerCase();
  const filtered = qLower ? all.filter(d => d.toLowerCase().includes(qLower)) : all;

  const exactMatch = q && all.some(d => d.toLowerCase() === qLower);
  const canAddNew = q.length > 0 && !exactMatch;

  let html = '';
  if (!filtered.length && !canAddNew) {
    html = '<div class="dept-search-empty">Department tidak ditemukan.</div>';
  } else {
    html = filtered.slice(0, 10).map(d =>
      `<div class="dept-search-option" data-dept="${escapeHtml(d)}">${escapeHtml(d)}</div>`
    ).join('');
  }

  if (canAddNew) {
    html += `<div class="dept-search-option dept-search-add-new" data-dept="${escapeHtml(q)}" style="color:#2563eb;font-weight:600;border-top:1px solid #e2e8f0;">➕ Tambah "${escapeHtml(q)}" sebagai department baru</div>`;
  }

  box.innerHTML = html;
  box.classList.remove('hidden');
  ensureDeptOptionsListener_();
}

let deptOptionsListenerAttached_ = false;
function ensureDeptOptionsListener_() {
  if (deptOptionsListenerAttached_) return;
  const box = document.getElementById('newEmployeeDeptOptions');
  box.addEventListener('mousedown', e => {
    const opt = e.target.closest('.dept-search-option[data-dept]');
    if (!opt) return;
    selectDeptOption_(opt.dataset.dept);
  });
  deptOptionsListenerAttached_ = true;
}

  function onDeptSearchFocus_() {
    renderDeptSearchOptions_(document.getElementById('newEmployeeDeptSearch').value);
  }

  function onDeptSearchInput_() {
    document.getElementById('newEmployeeDept').value = '';
    renderDeptSearchOptions_(document.getElementById('newEmployeeDeptSearch').value);
  }

  function onDeptSearchBlur_() {
    setTimeout(() => {
      if (deptSelectionInProgress_) { deptSelectionInProgress_ = false; return; }
      document.getElementById('newEmployeeDeptOptions').classList.add('hidden');
      if (!document.getElementById('newEmployeeDept').value) {
        document.getElementById('newEmployeeDeptSearch').value = '';
      }
    }, 150);
  }

  function selectDeptOption_(dept) {
    deptSelectionInProgress_ = true;
    document.getElementById('newEmployeeDept').value = dept;
    document.getElementById('newEmployeeDeptSearch').value = dept;
    document.getElementById('newEmployeeDeptOptions').classList.add('hidden');
  }
  ...
  if (!department) {
    document.getElementById('addEmployeeError').textContent = 'Pilih Department dari daftar yang tersedia (tidak bisa ketik bebas).';
    return;
  }
...
function openDeleteEmployeePicker() {
  ...
  document.getElementById('deleteEmployeeSearch').value = '';
  document.getElementById('deleteEmployeeSelect').value = '';
  document.getElementById('deleteEmployeeSearchOptions').classList.add('hidden');
...
function populateDeleteEmployeeOptions_() {
  DELETE_EMPLOYEE_LIST_ = (STATE.rekap || []).filter(r => r.nama && String(r.nama).trim() !== '');
}

/* ---- Searchable Dropdown Hapus Karyawan ---- */

let deleteEmployeeSelectionInProgress_ = false;
let deleteEmployeeOptionsListenerAttached_ = false;

function renderDeleteEmployeeSearchOptions_(query) {
  const box = document.getElementById('deleteEmployeeSearchOptions');
  const q = (query || '').trim().toLowerCase();
  const filtered = q
    ? DELETE_EMPLOYEE_LIST_.filter(r => String(r.nama).toLowerCase().includes(q))
    : DELETE_EMPLOYEE_LIST_;

  if (!filtered.length) {
    box.innerHTML = '<div class="dept-search-empty">Karyawan tidak ditemukan.</div>';
  } else {
    box.innerHTML = filtered.slice(0, 10).map(r =>
      `<div class="dept-search-option" data-nama="${escapeHtml(r.nama)}">${escapeHtml(r.nama)} (${escapeHtml(r.jabatan)})</div>`
    ).join('');
  }
  box.classList.remove('hidden');
  ensureDeleteEmployeeOptionsListener_();
}

function ensureDeleteEmployeeOptionsListener_() {
  if (deleteEmployeeOptionsListenerAttached_) return;
  const box = document.getElementById('deleteEmployeeSearchOptions');
  box.addEventListener('mousedown', e => {
    const opt = e.target.closest('.dept-search-option[data-nama]');
    if (!opt) return;
    selectDeleteEmployeeOption_(opt.dataset.nama);
  });
  deleteEmployeeOptionsListenerAttached_ = true;
}

function onDeleteEmployeeSearchFocus_() {
  renderDeleteEmployeeSearchOptions_(document.getElementById('deleteEmployeeSearch').value);
}

function onDeleteEmployeeSearchInput_() {
  document.getElementById('deleteEmployeeSelect').value = '';
  renderDeleteEmployeeSearchOptions_(document.getElementById('deleteEmployeeSearch').value);
}

function onDeleteEmployeeSearchBlur_() {
  setTimeout(() => {
    if (deleteEmployeeSelectionInProgress_) { deleteEmployeeSelectionInProgress_ = false; return; }
    document.getElementById('deleteEmployeeSearchOptions').classList.add('hidden');
    if (!document.getElementById('deleteEmployeeSelect').value) {
      document.getElementById('deleteEmployeeSearch').value = '';
    }
  }, 150);
}

function selectDeleteEmployeeOption_(nama) {
  deleteEmployeeSelectionInProgress_ = true;
  document.getElementById('deleteEmployeeSelect').value = nama;
  document.getElementById('deleteEmployeeSearch').value = nama;
  document.getElementById('deleteEmployeeSearchOptions').classList.add('hidden');
}
...
function submitDeleteEmployee() {
  ...
  function renderDaftarKaryawanTable() {
    ...
    const cols = ['NIK', 'No HP', 'Nama', 'Department', 'Jabatan', 'Email', 'Status Karyawan', 'Tanggal Registrasi', 'Status Jadwal'];
    ...
    DAFTAR_KARYAWAN_DATA.forEach(k => {
      ...
      html += `<tr data-name="${escapeHtml((k.nama || '').toLowerCase())}">
        ...
        <td>${k.noHp ? escapeHtml(k.noHp) : '-'}</td>
  ...
  function openEditKaryawanPicker(nama) {
    ...
    document.getElementById('karyawanNoHpInput').value = k.noHp || '';
    document.getElementById('karyawanEmailInput').value = k.email || '';
  ...
  function submitKaryawanData() {
    ...
    const fields = {
      ...
      noHp: document.getElementById('karyawanNoHpInput').value.trim(),
      email: document.getElementById('karyawanEmailInput').value.trim(),
...
/* ============ GANTI PASSWORD ============ */
  ...
  function downloadPdf(sheetName) {
    ...
    // Dashboard & Rekap berisi rumus TOTAL gabungan lintas department -> tidak
    // dikirim department (backend akan minta pakai menu Report kalau tetap dipaksa filter).
    const isAggregateSheet = (sheetName === 'Dashboard' || sheetName === 'Rekap');
    const deptToSend = isAggregateSheet ? 'ALL' : currentDeptFilter;
    ...
    google.script.run
      ...
      .exportSheetPdf(sheetName, SESSION ? SESSION.token : null, deptToSend);
  }
...
function sendAIChatMessage() {
  ...
  google.script.run
    .withSuccessHandler(perms => {
      MY_PERMISSIONS_ = (perms && perms.view) ? perms : { view: (perms || []), edit: (perms || []) };
      const allowedSet = new Set(MY_PERMISSIONS_.view);
  ...
  if (!SESSION || SESSION.role !== 'Super Admin') { toast('Hanya Super Admin yang boleh menambah user', true); return; }
  document.getElementById('newUserNama').value = '';
  document.getElementById('newUserUsername').value = '';
  document.getElementById('newUserPassword').value = '';
  document.getElementById('addUserError').textContent = '';
  const sel = document.getElementById('newUserRole');
  sel.innerHTML = '<option value="">Memuat daftar role...</option>';
  google.script.run
    .withSuccessHandler(roles => {
      const list = (roles || []).filter(r => r.roleName);
      sel.innerHTML = list.length
        ? list.map(r => `<option value="${escapeHtml(r.roleName)}">${escapeHtml(r.roleName)}</option>`).join('')
        : '<option value="">(Belum ada role — buat dulu di Setting &gt; Hak Akses)</option>';
    })
    .withFailureHandler(() => {
      sel.innerHTML = '<option value="Staff">Staff</option><option value="Admin">Admin</option><option value="Super Admin">Super Admin</option>';
    })
...
function saveJamKerjaSettings() {
  ...
  const permMap = {};
  HAKAKSES_DATA.permissions.forEach(p => { permMap[p.roleName + '|' + p.menuKey] = { view: !!p.canView, edit: !!p.canEdit }; });

  if (!roles.length) {
    document.getElementById('settingHakAksesWrap').innerHTML = '<p class="muted">Belum ada role selain Super Admin. Klik "➕ Tambah Role" untuk membuat role baru (misal: Viewer).</p>';
    return;
  }

  let html = '<p class="muted" style="margin-bottom:10px;">Centang <strong>Lihat</strong> untuk menampilkan menu di sidebar. Centang <strong>Edit</strong> untuk mengizinkan simpan perubahan (saat ini berlaku efektif untuk menu "Input Realisasi").</p>';
  html += '<table><thead><tr><th class="name-col">Menu</th>' + roles.map(r => `<th colspan="2">${escapeHtml(r.roleName)}</th>`).join('') + '</tr>';
  html += '<tr><th class="name-col"></th>' + roles.map(() => '<th>Lihat</th><th>Edit</th>').join('') + '</tr></thead><tbody>';
  menus.forEach(m => {
    html += `<tr><td class="name-col">${escapeHtml(m.label)}</td>`;
    roles.forEach(r => {
      const p = permMap[r.roleName + '|' + m.key] || { view: false, edit: false };
      html += `<td><input type="checkbox" class="hakakses-view-chk" data-role="${escapeHtml(r.roleName)}" data-menu="${m.key}" ${p.view ? 'checked' : ''} onchange="onHakAksesViewToggle_(this)"></td>`;
      html += `<td><input type="checkbox" class="hakakses-edit-chk" data-role="${escapeHtml(r.roleName)}" data-menu="${m.key}" ${p.edit ? 'checked' : ''}></td>`;
    });
    html += '</tr>';
  });
  html += '</tbody></table>';
...
/** Kalau "Lihat" dimatikan, otomatis matikan "Edit" juga (edit tanpa lihat tidak masuk akal). */
function onHakAksesViewToggle_(viewChk) {
  if (viewChk.checked) return;
  const role = viewChk.dataset.role, menu = viewChk.dataset.menu;
  const editChk = document.querySelector(`.hakakses-edit-chk[data-role="${CSS.escape(role)}"][data-menu="${menu}"]`);
  ...
  const tasks = roles.map(r => ({
    roleName: r.roleName,
    menuKeys: Array.from(table.querySelectorAll(`.hakakses-view-chk[data-role="${CSS.escape(r.roleName)}"]:checked`)).map(cb => cb.dataset.menu),
    editMenuKeys: Array.from(table.querySelectorAll(`.hakakses-edit-chk[data-role="${CSS.escape(r.roleName)}"]:checked`)).map(cb => cb.dataset.menu)
  }));
  ...
  function runNext(i) {
    ...
    google.script.run
      ...
      .setRolePermissions(tasks[i].roleName, tasks[i].menuKeys, tasks[i].editMenuKeys, SESSION.token);
  }
  runNext(0);
}
...
const NOTIF_LAST_SEEN_KEY = 'ams_notif_last_seen';
let notifCache = [];
...
function startNotifPolling_() {
  loadNotifications_();
  if (notifPollTimer) clearInterval(notifPollTimer);
  notifPollTimer = setInterval(loadNotifications_, 30000);
}

function getNotifLastSeen_() {
  return parseInt(localStorage.getItem(NOTIF_LAST_SEEN_KEY) || '0', 10);
}
function setNotifLastSeenNow_() {
...
function loadNotifications_() {
  ...
  google.script.run
    .withSuccessHandler(res => {
      ...
      notifCache = res.notifications || [];
      renderNotifBadge_();
      renderNotifList_();
    })
    .withFailureHandler(err => {
      console.error('loadNotifications_ error', err);
      if (isAuthError_(err.message)) doLogout();
    })
...
function renderNotifBadge_() {
  const lastSeen = getNotifLastSeen_();
  const unread = notifCache.filter(n => n.timestamp > lastSeen).length;
...
function renderNotifList_() {
  ...
  if (notifCache.length === 0) {
    list.innerHTML = '<div class="muted" style="padding:16px;text-align:center;">Tidak ada notifikasi</div>';
    return;
  }
  list.innerHTML = notifCache.map(n => {
    ...
    return '<div class="notif-item' + (isUnread ? ' unread' : '') + '">' +
      ...
      '<div class="notif-time">' + new Date(n.timestamp).toLocaleString('id-ID') + '</div>' +
      '</div>';
  }).join('');
}

function initNotifBell_() {
  const bellBtn = document.getElementById('notifBellBtn');
  const markAllBtn = document.getElementById('notifMarkAllBtn');
  if (!bellBtn || !markAllBtn) return;
